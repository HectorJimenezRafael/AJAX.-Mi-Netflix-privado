...
function ListarUsuariosCrud(buscar_id, buscar_nombre, buscar_correo, buscar_estado) {
    ...
    ajax.open('POST', '../cruds/listar_usuarios.php');
...
registrar.addEventListener("click", () => {
    ...
    var ajax = new XMLHttpRequest();
    ajax.open('POST', '../cruds/crear_usuario.php');
    ajax.onload = function() {
        if (ajax.status === 200) {
...
function Eliminar(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            formdata.append('id', id);
            var ajax = new XMLHttpRequest();
            ajax.open('POST', '../cruds/eliminar_usuario.php');
            ajax.onload = function() {
                if (ajax.status === 200) {
...
function Editar(id) {
    ...
    formdata.append('id', id);
    var ajax = new XMLHttpRequest();
    ajax.open('POST', '../cruds/editar_usuario.php');
    ajax.onload = function() {
        if (ajax.status == 200) {
            var json = JSON.parse(ajax.responseText);
            // alert(json);
            document.getElementById('idp').value = json.id;
            document.getElementById('nombre').value = json.nombre_usu;
            document.getElementById('correo').value = json.correo;
            document.getElementById('estado').value = json.habilitado;
            // document.getElementById('lugar').value = json.id_lugar;
            // document.getElementById('capacidad').value = json.capacidad;
