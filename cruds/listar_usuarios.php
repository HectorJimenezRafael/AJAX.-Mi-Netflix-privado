...
if(!empty($_POST['buscar_nombre']) ){
    ...
    $consulta = $pdo->prepare("SELECT * FROM `tbl_usuarios` WHERE `admin`=0 AND `nuevo`=0  AND nombre_usu LIKE '%".$flitro_nombre."%' AND correo LIKE '%".$flitro_correo."%' AND id LIKE '%".$flitro_id."%' AND habilitado LIKE '%".$flitro_estado."%'");
    $consulta->execute();
 ...
 elseif (empty($_POST['buscar_nombre']) && empty($_POST['buscar_correo']) && empty($_POST['buscar_estado']) && empty($_POST['buscar_id'])) {
    $consulta = $pdo->prepare("SELECT * FROM `tbl_usuarios` WHERE `admin`=0 AND `nuevo`=0 ");
$consulta->execute();
}
